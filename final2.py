...
# Estructura de Datos:
evaluations = data()  # Lista de evaluaciones EIT con sus datos (por ejemplo, ID, semestre, preferencias de horario, etc.).

# Nuevos bloques horarios
...
def resolve_same_semester_same_day(schedule):
    ...
    # Elegir dos evaluaciones aleatorias en esos días
    eval1 = random.choice(neighbor_solution[day1])
    eval2 = random.choice(neighbor_solution[day2])

    # Intercambiar las evaluaciones entre los dos días
    neighbor_solution[day1].remove(eval1)
    neighbor_solution[day2].remove(eval2)
    neighbor_solution[day1].append(eval2)
    neighbor_solution[day2].append(eval1)
